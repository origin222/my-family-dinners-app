...
const PLAN_RESPONSE_SCHEMA = { type: "OBJECT", properties: { "weeklyPlan": { type: "ARRAY", description: "A 7-day dinner plan.", items: { type: "OBJECT", properties: { "day": { "type": "STRING" }, "meal": { "type": "STRING" }, "description": { "type": "STRING" } } } }, "shoppingList": { type: "ARRAY", description: "A consolidated list of all ingredients.", items: { type: "OBJECT", properties: { "item": { "type": "STRING" }, "quantity": { "type": "STRING" }, "category": { "type": "STRING" }, "isChecked": { "type": "BOOLEAN" } }, propertyOrdering: ["item", "quantity", "category", "isChecked"] } } }, propertyOrdering: ["weeklyPlan", "shoppingList"] };
...
const App = () => {
    ...
    const processPlanGeneration = useCallback(async (isRegeneration = false) => { if (!db || !userId || isLoading || !query.trim()) return; setIsLoading(true); setError(null); setDetailedRecipe(null); setSelectedMealIndex(null); setMealsToRegenerate([]); const oldPlan = planData; let systemPrompt; let userPrompt = "Generate the complete weekly dinner plan and consolidated shopping list."; if (isRegeneration && oldPlan) { const mealsToUpdate = mealsToRegenerate.map(index => oldPlan.weeklyPlan[index].day).join(', '); const unchangedMeals = oldPlan.weeklyPlan.filter((_, index) => !mealsToRegenerate.includes(index)).map(meal => `${meal.day}: ${meal.meal} (${meal.description})`).join('; '); systemPrompt = `You are updating a meal plan based on: "${oldPlan.initialQuery}". Generate NEW meals for: ${mealsToUpdate}. Keep these meals: ${unchangedMeals}. New meals must follow this constraint: ${regenerationConstraint || 'None'}. Generate a single 'shoppingList' for ALL 7 final meals.`; userPrompt = `Replace meals for ${mealsToUpdate}. Return the full 7-day plan with a new consolidated shopping list.`; setRegenerationConstraint(''); } else { systemPrompt = `You are a meal planner. Generate a 7-day dinner plan and shopping list based on: "${query.trim()}". Provide exactly 7 meals. Consolidate ALL ingredients into a single 'shoppingList'. 'isChecked' must be 'false'.`; } try { const payload = { contents: [{ parts: [{ text: userPrompt }] }], systemInstruction: { parts: [{ text: systemPrompt }] }, generationConfig: { responseMimeType: "application/json", responseSchema: PLAN_RESPONSE_SCHEMA } }; const url = `${API_URL}?key=${finalGeminiApiKey}`; const response = await retryFetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!response.ok) { throw new Error(`API error: ${response.statusText}`); } const result = await response.json(); const jsonString = result.candidates?.[0]?.content?.parts?.[0]?.text; if (!jsonString) { throw new Error("AI response was empty."); } const parsedPlan = JSON.parse(jsonString); const mergedList = mergeShoppingLists(parsedPlan.shoppingList, oldPlan?.shoppingList); const newPlanData = { ...parsedPlan, shoppingList: mergedList, initialQuery: query.trim() }; const docRef = doc(db, 'artifacts', appId, 'users', userId, 'mealPlans', MEAL_PLAN_DOC_ID); await setDoc(docRef, newPlanData); setView('review'); } catch (e) { console.error("Plan Generation Error:", e); setError(`Failed to generate plan: ${e.message}.`); } finally { setIsLoading(false); setMealsToRegenerate([]); } }, [db, userId, query, planData, mealsToRegenerate, regenerationConstraint, retryFetch]);
    ...
    // --- UI TWEAK: Dark Mode Toggle Component ---
    const ThemeToggle = () => {
        ...
        return (
            <label className="grid cursor-pointer place-items-center">
                <input type="checkbox" onChange={handleToggle} checked={theme === 'dark'} className="toggle theme-controller bg-base-content col-span-2 col-start-1 row-start-1" />
                ...
                <svg className="stroke-base-100 fill-base-100 col-start-2 row-start-1" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" /></svg>
            </label>
        );
    };
            ...
            <div className="max-w-5xl mx-auto bg-base-100 rounded-box shadow-2xl p-6 sm:p-10">
                ...
                {planData && (
                    <div className="flex justify-center mb-8">
                        <div className="join">
                            <button onClick={() => setView('review')} disabled={!planData} className={`join-item btn ${['review', 'timing', 'detail'].includes(view) ? 'btn-primary' : ''}`}>Dinner Plan</button>

...
const VERCEL_FIREBASE_CONFIG_STRING_RAW = import.meta.env.VITE_FIREBASE_CONFIG;
console.log("Raw VITE_FIREBASE_CONFIG string from import.meta.env:", VERCEL_FIREBASE_CONFIG_STRING_RAW);
// --- END: DEBUG LOGGING ---
...
try {
    if (VERCEL_FIREBASE_CONFIG_STRING) {
        ...
        // --- START: DEBUG LOGGING ---
        console.log("Successfully parsed firebaseConfig:", firebaseConfig);
        // --- END: DEBUG LOGGING ---
    } else {
        // --- START: DEBUG LOGGING ---
        console.log("VITE_FIREBASE_CONFIG was empty or undefined, could not parse.");
        // --- END: DEBUG LOGGING ---
    }
} catch (e) {
    console.error("CRITICAL: Error parsing VITE_FIREBASE_CONFIG JSON:", e);
    // --- START: DEBUG LOGGING ---
    console.log("The JSON.parse method failed. The string is likely malformed.");
    // --- END: DEBUG LOGGING ---
}
...
const App = () => {
    ...
    const updateShoppingList = useCallback(async (updatedList) => { if (!db || !userId || !planData) return; const docRef = doc(db, 'artifacts', appId, 'users', userId, 'mealPlans', MEAL_PLAN_DOC_ID); try { await updateDoc(docRef, { shoppingList: updatedList }); } catch (e) { console.error("Firestore Update Error:", e); } }, [db, userId, planData, appId]);
    ...
    const processPlanGeneration = useCallback(async (isRegeneration = false) => {
        if (!db || !userId) {
            toast.error("Not connected to the database. Please refresh.");
            return;
        }
        if (isLoading) return;
        if (!query.trim() && !isRegeneration) {
            toast.error("Please enter your family's preferences first.");
            return;
        }
    ...
    }, [db, userId, query, isLoading /* ... other dependencies */]);
